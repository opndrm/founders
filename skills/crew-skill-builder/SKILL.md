---
name: crew-skill-builder
description: Build, review, and improve reliable, portable Crew skills for approved agent harnesses. Use when the Captain asks to create, rename, customize, install, distribute, audit, or debug a Crew skill or its SKILL.md instructions. Applies the Crew visible-terminal and JCode ownership boundaries while keeping each skill focused and testable.
---

# Crew Skill Builder

Use this guide whenever the Captain asks for a new or revised Crew skill. Build skills that make the correct path easier to follow without silently expanding an AI's authority.

## Crew-specific rules

Before changing a Crew skill:

1. Identify whether the Captain wants a source-library copy, an installation for one named harness, or both. Do not assume one means the other.
2. Preserve the original source unless the Captain explicitly asks to modify it.
3. For a new installed skill, use that harness's standard skill-creation flow, keep the folder and frontmatter consistent, add only the metadata that harness requires, and validate the finished skill.
4. Keep one concern per skill. Use **Crew** for visible JCode workflow coordination, **Crew HERDR** for the canonical HERDR layout, **Crew Audit** for read-only evidence, **Crew Scheduler** for read-only monitoring, and **Crew Distribute** for approved skill synchronization. The visible workspace standard is HERDR in WezTerm.
5. Preserve JCode ownership: skills may guide visible terminal coordination and Captain-authorized messages, but must not authorize Crew code changes, tests, reviews, commits, pushes, pull requests, or No Mistakes pipeline control by the coordinating AI.
6. Treat a visible terminal result as the only proof of an interactive JCode or No Mistakes action. Hidden shell output is not equivalent.

Then use the complete guide below to design, validate, and maintain the skill.
