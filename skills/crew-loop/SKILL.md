---
name: crew-loop
description: Explain and write effective instructions for the `/goal` feature — the persistent self-checking agent loop (plan → act → test → review → iterate). Use when the Captain mentions Crew Loop, `/goal`, goal loop, Ralph loop, a long-running autonomous agent run, a goal prompt, or a one-paragraph goal instruction.
---
...
`/goal` is one example of a persistent-agent feature: it turns a prompt into a loop of `plan → act → test → review → iterate` until a stop condition is met, the user pauses, or the task limit is reached. Some communities call this a "Ralph loop."

Persistent-loop support differs by harness. Inspect the active harness's capabilities and lifecycle controls before starting one; do not assume a slash command, authentication model, or resume behavior is universal.
...
- An active harness with a persistent-loop feature
- That feature enabled and its pause, stop, and status controls verified
- An authentication and quota model that supports the intended duration
...
Hand-written goals under-specify. Ask an independent agent session with the codebase loaded to: (1) inspect the codebase, (2) surface hidden assumptions, constraints, and edge cases, and (3) emit a structured goal contract. Use that contract with the active harness's persistent-loop feature.
