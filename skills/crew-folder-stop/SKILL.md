---
name: crew-folder-stop
description: Configure and maintain a configurable safety stop for catastrophic shell commands while AI agents work in approved folders or repositories. Use when the Captain asks for command protection, destructive-command blocking, guardrails, dangerous-command hooks, PreToolUse safety, or an audit of existing AI terminal safeguards. Keep normal development and read-only work usable; require explicit approval before changing machine-wide protections.
---

# Crew Folder Stop

Use this skill to set up or review an enforceable safety stop for AI terminal commands. It is deliberately general: use it for Crew or another approved repository without hard-coding one workflow, model, terminal app, or folder path.

## Operating boundary

- Protect against genuinely catastrophic or difficult-to-recover actions, such as broad deletion, disk formatting, repository deletion, unsafe force-pushes, and obvious credential-exfiltration launchers.
- Do not turn ordinary development, diagnosis, or Captain-approved work into a wall of false alarms.
- Treat the denylist as a last line of defense, not as a replacement for repository instructions, review, or user approval.
- Inspect the active tool and hook support before proposing configuration. Agent harnesses differ and may not expose a pre-command safety hook.
- Present the affected agents, exact protection scope, and rollback plan before changing any shared hook, configuration, or blocklist. Apply only after the Captain approves.
- Re-test the guard using safe probes after installation or a rule change. If a rule blocks legitimate normal work, narrow it instead of disabling all protection.

## How it fits with folder rails

**Crew Folder Rails** gives an AI instructions for operating safely in a repository. **Crew Folder Stop** is independent enforcement: it can block a catastrophic shell command even when an AI misunderstood those instructions. Use both where the risk justifies it; neither one replaces the other.
